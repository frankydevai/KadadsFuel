...
# Duplicate flag cooldown — don't re-flag same truck+type+stop within this window
...
def _ensure_flags_table():
    ...
    with db_cursor() as cur:
        cur.execute(
            ...
            ALTER TABLE driver_flags ADD COLUMN IF NOT EXISTS card_price REAL;
            ALTER TABLE driver_flags ADD COLUMN IF NOT EXISTS state TEXT;
            """
        )


def _is_duplicate_flag(
    vehicle_name: str,
    flag_type: str,
    recommended_stop: str = None,
) -> bool:
    """Check if an identical flag was already raised recently (within cooldown)."""
    try:
        with db_cursor() as cur:
            cur.execute(
                """
                ...
                WHERE vehicle_name = %s
                  AND flag_type = %s
                  AND (recommended_stop = %s OR (%s IS NULL AND recommended_stop IS NULL))
                  AND flagged_at >= NOW() - INTERVAL '1 minute' * %s
                """,
                (vehicle_name, flag_type, recommended_stop,
                 recommended_stop, DUPLICATE_COOLDOWN_MINUTES),
            )
...
def save_flag(
    ...
    # Duplicate prevention — skip if same flag exists within cooldown
    if _is_duplicate_flag(vehicle_name, flag_type, recommended_stop):
        log.info(f"Duplicate flag suppressed: {flag_type} for {vehicle_name} at {recommended_stop}")
        return -1
...
def flag_wrong_stop(
    ...
    flag_id = save_flag(
        ...
        card_price=rec_card_price,
        savings_lost=savings_lost if savings_lost > 0 else None,
    )
...
def get_flags_for_excel(days: int = 7) -> list:
    """Get all flag records for the weekly Excel report.

    Returns full detail rows with all columns needed for customer-facing report.
    """
    _ensure_flags_table()
    from datetime import datetime, timezone, timedelta
    ...
    with db_cursor() as cur:
        cur.execute(
            """
            SELECT id, vehicle_name, flag_type, details,
